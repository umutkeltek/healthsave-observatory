"""One-time CLI Google Health API authorization-code grant."""
...
async def run_authorize_flow(
    *,
    code: str,
    oauth_config: GoogleHealthClientConfig,
    http_client,
    session,
    token_store,
...
async def _interactive_main(owner_id: UUID = DEFAULT_OWNER_ID) -> int:  # pragma: no cover
    ...
    try:
        oauth_config = GoogleHealthClientConfig.from_env()
    ...
    print("Open URL in browser, grant access, paste 'code'")
    print("query parameter from redirect URL below:")
    print()
    print(f"  {url}")
    print()
    ...
    print(f"stored Google Health token for owner={token.owner_id} scopes={','.join(token.scopes)}")
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--owner-id",
        default=str(DEFAULT_OWNER_ID),
        help="Owner UUID for stored token row.",
    )
    args = parser.parse_args(argv)
    return asyncio.run(_interactive_main(UUID(args.owner_id)))
