...
folded into ``daily_activity`` rather than the catch-all ``quantity_samples``
table.

ARCH-001: this config drives the storage writers, so it lives below the storage
layer; the API layer + plugins reach it via the ``server.ingestion.mappers``
re-export shim.
"""

from normalization.parsers import normalize_blood_oxygen, to_float, to_int
...
DEDICATED_TABLES = {
    ...
    "blood_oxygen": {
        "table": "blood_oxygen",
        "columns": {"date": "time", "qty": "spo2_pct", "source": "source_id"},
    ...
    "body_temperature": {
        ...
        "columns": {"date": "time", "qty": "temp_celsius", "source": "source_id"},
